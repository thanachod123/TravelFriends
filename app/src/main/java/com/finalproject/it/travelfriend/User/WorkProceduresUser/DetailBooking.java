package com.finalproject.it.travelfriend.User.WorkProceduresUser;
...
public class DetailBooking extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnCallGuide = findViewById(R.id.btn_call_guide);
        btnEmergencyCall = findViewById(R.id.btn_emergency_call);

        mDatabase = FirebaseDatabase.getInstance();
        mReferencePackage = mDatabase.getReference().child("Packages");
        mReferenceGuide = mDatabase.getReference("Users");
        mReferenceBooking = mDatabase.getReference().child("Booking");
        bookingId = getIntent().getStringExtra("BookingId");
        bindData();

        btnCallGuide.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent callGuide = new Intent(Intent.ACTION_CALL);
                callGuide.setData(Uri.parse("tel:" + strGuidePhone));
                if (ActivityCompat.checkSelfPermission(DetailBooking.this, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(DetailBooking.this,new String[] {Manifest.permission.CALL_PHONE}, REQUEST_CALL);
                    return;
                }
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                mReferencePackage.child(packageId).addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        Picasso.with(DetailBooking.this).load(strPackageImage).placeholder(R.drawable.package_image).into(img_package);
                    }
                ...
                mReferenceGuide.child(guideId).addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        strGuidePhone = dataSnapshot.child("phone").getValue(String.class);
                        txtNameGuide.setText(strGuideName + " " + strGuideSurname);
                        Picasso.with(DetailBooking.this).load(strGuideImage).placeholder(R.drawable.default_profile).into(img_guide_image);
                    }
